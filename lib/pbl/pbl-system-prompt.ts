...
export function buildPBLSystemPrompt(config: PBLSystemPromptConfig): string {
  ...
  // zh-CN falls through to the English template with Chinese language instruction
  // (Chinese system prompt removed — the English template handles all languages)
  ...
  const languageName = language === 'nl-NL' ? 'Dutch (Nederlands)' : language === 'en-US' ? 'English' : language === 'zh-CN' ? 'Chinese (Simplified)' : language;
...
/**
 * @deprecated Chinese system prompt removed. The zh-CN case now falls through
 * to the English template with a language instruction for Chinese output.
 * This stub is kept for reference; it can be deleted in a future cleanup.
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function _buildPBLSystemPromptZH_removed(_config: PBLSystemPromptConfig): never {
  throw new Error('Chinese system prompt has been removed. Use the English template with language instruction.');
}
