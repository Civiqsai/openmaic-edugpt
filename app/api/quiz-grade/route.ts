...
{"score": <0到${points}的整数>, "comment": "<一两句评语>"}`
      : isNl
        ? `Je bent een professionele onderwijsbeoordelaar. Beoordeel het antwoord van de student en geef kort feedback.
Antwoord uitsluitend in het volgende JSON-formaat (geen andere inhoud):
{"score": <geheel getal van 0 tot ${points}>, "comment": "<een of twee zinnen feedback>"}`
...
${commentPrompt ? `评分要点：${commentPrompt}\n` : ''}学生答案：${userAnswer}`
      : isNl
        ? `Vraag: ${question}
Maximale score: ${points} punten
${commentPrompt ? `Beoordelingsrichtlijn: ${commentPrompt}\n` : ''}Antwoord student: ${userAnswer}`
        : `Question: ${question}
    ...
    try {
      ...
      gradeResult = {
        ...
        comment: isZh
          ? '已作答，请参考标准答案。'
          : isNl
            ? 'Antwoord ontvangen. Raadpleeg het standaardantwoord.'
